...
class Custom_py(QsciLexerPython):
	def __init__(self, *args):
		...
		self.setColor(QColor("#8264B7"), QsciLexerPython.Number)
	...
	def keywords(self, ens):
		if ens == 1:
			return "and as assert break class continue def del elif else except exec finally for from global if import in is lambda None not or pass print raise return try while with yield"
		elif ens == 2:
